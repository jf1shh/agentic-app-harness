...
// Returns null if the section heading cannot be found at all.
export function extractLearnedLessonsSection(agentsMdContent) {
  const allLines = agentsMdContent.split(/\r?\n/);
...
// Every [guardrail: id] tag within the given lines, mapped to its line text.
export function parseTaggedLines(lines) {
  const taggedLines = new Map();
...
// The four traceability checks described in the file header. Pure: takes the
// guardrail list and the already-parsed tag map, returns an array of human-
// readable error strings (empty when the loop is fully closed).
export function checkTraceability(guardrails, taggedLines) {
...
if (invokedDirectly) {
  ...
  const lines = extractLearnedLessonsSection(agents);
  if (lines === null) {
    console.error("Could not find the '## 6.' Learned Lessons section in AGENTS.md.");
    process.exit(1);
  }

  const taggedLines = parseTaggedLines(lines);
  const errors = checkTraceability(GUARDRAILS, taggedLines);
