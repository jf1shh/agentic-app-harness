...
| File | Role |
|---|---|
| `goldenset.json` | Single source of truth: 12 counsel queries → the document (+ section) that must be retrieved. Derived from `src/lib/datasets/authenticSampleDocs.ts`. |
...
Current baseline: **precision@K 100%, MRR 1.000** (every expected doc ranks #1),
so the floors leave headroom to catch a real regression without flaking.
