...
- **Mandatory Remote Deployment Verification**: Whenever initiating or triggering remote CI/CD workflows or cloud deployments (e.g., GitHub Actions workflows, Vercel builds), agents MUST actively poll, inspect, and verify the deployment job status until it reaches `completed success` before reporting completion to the user.
...
- **Update Documentation**: At the end of every session or major milestone, agents MUST update all relevant `.md` files (e.g., project specifications in `specs/`, walkthroughs, implementation plans, and READMEs) to accurately reflect the latest project state, architecture, and completed features.
