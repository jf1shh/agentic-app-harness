...
artifacts, spec drift, and ~20 line-level "guardrails" distilled from real bugs (each one traces to a
...
and `validate-specs.ps1 -Strict` on every PR; `deploy-pages.yml` and `android-release.yml` build and ship
