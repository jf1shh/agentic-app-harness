# Work Order: No release signing config in mood-diner android/app/build.gradle
...
- **Finding ID:** `mood-diner-mobile-no-signing-config`
- **App:** `mood-diner`
...
3. The acceptance gate above must pass:
   ...
   - `validate-specs*` → `node scripts/harness-status.mjs --strict` no longer reports `mood-diner-mobile-no-signing-config`.
