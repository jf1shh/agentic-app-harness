# Work Order: 1/1 unit test file(s) not BDD-formatted in projects/portfolio-hub
...
- **Finding ID:** `portfolio-hub-unit-bdd-noncompliant`
- **App:** `portfolio-hub`
...
- `projects/portfolio-hub/src/utils/__tests__/portfolioData.test.ts:1` — `no Given/When/Then`
...
3. The acceptance gate above must pass:
   ...
   - `validate-specs*` → `node scripts/harness-status.mjs --strict` no longer reports `portfolio-hub-unit-bdd-noncompliant`.
