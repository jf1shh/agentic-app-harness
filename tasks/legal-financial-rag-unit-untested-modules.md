# Work Order: 1/12 core-logic module(s) have no unit test in projects/legal-financial-rag
...
- **Finding ID:** `legal-financial-rag-unit-untested-modules`
...
- `projects/legal-financial-rag/src/lib/hooks/useAutoLock.ts:1` — `no unit test imports this module`
...
3. The acceptance gate above must pass:
   ...
   - `validate-specs*` → `node scripts/harness-status.mjs --strict` no longer reports `legal-financial-rag-unit-untested-modules`.
