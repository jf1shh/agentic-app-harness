# Work Order: 2/7 core-logic module(s) have no unit test in projects/mood-diner
...
- **Finding ID:** `mood-diner-unit-untested-modules`
- **App:** `mood-diner`
...
- `projects/mood-diner/src/lib/schemas.ts:1` — `no unit test imports this module`
- `projects/mood-diner/src/data/restaurantsData.ts:1` — `no unit test imports this module`
...
3. The acceptance gate above must pass:
   ...
   - `validate-specs*` → `node scripts/harness-status.mjs --strict` no longer reports `mood-diner-unit-untested-modules`.
