# Work Order: No CI job builds the mood-diner Android artifact
...
- **Finding ID:** `mood-diner-mobile-no-android-ci`
- **App:** `mood-diner`
...
3. The acceptance gate above must pass:
   ...
   - `validate-specs*` → `node scripts/harness-status.mjs --strict` no longer reports `mood-diner-mobile-no-android-ci`.
