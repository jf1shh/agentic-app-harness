# Work Order: 1/4 core-logic module(s) have no unit test in projects/smart-recipe-app
...
- **Finding ID:** `smart-recipe-app-unit-untested-modules`
...
- `projects/smart-recipe-app/src/lib/rag/schemas.ts:1` — `no unit test imports this module`
...
3. The acceptance gate above must pass:
   ...
   - `validate-specs*` → `node scripts/harness-status.mjs --strict` no longer reports `smart-recipe-app-unit-untested-modules`.
